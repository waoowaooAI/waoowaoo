...
export function areStoryboardsEquivalent(
  previous: ProjectStoryboard[],
  next: ProjectStoryboard[],
): boolean {
...
export function computeTotalPanels(storyboards: ProjectStoryboard[]): number {
  return storyboards.reduce((sum, storyboard) => sum + (getStoryboardPanels(storyboard).length || storyboard.panelCount || 0), 0)
}

export function formatClipTitle(clip: ProjectClip | null | undefined): string {
  if (!clip) return '-'
