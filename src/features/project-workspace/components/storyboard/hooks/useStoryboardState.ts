...
import { ProjectStoryboard, ProjectClip, ProjectPanel } from '@/types/project'
...
export function useStoryboardState({
  ...
  const setLocalStoryboards = useCallback<React.Dispatch<React.SetStateAction<ProjectStoryboard[]>>>(
    (nextStoryboardsOrUpdater) => {
      const resolveNextStoryboards = (previousStoryboards: ProjectStoryboard[]) => (
        typeof nextStoryboardsOrUpdater === 'function'
          ? (nextStoryboardsOrUpdater as (previous: ProjectStoryboard[]) => ProjectStoryboard[])(previousStoryboards)
          : nextStoryboardsOrUpdater
      )

      queryClient.setQueryData(queryKeys.episodeData(projectId, episodeId), (previous: unknown) => {
        if (!previous || typeof previous !== 'object') return previous
        const episode = previous as { storyboards?: ProjectStoryboard[] }
      ...
      queryClient.setQueryData(queryKeys.storyboards.all(episodeId), (previous: unknown) => {
        if (!previous || typeof previous !== 'object') return previous
        const payload = previous as { storyboards?: ProjectStoryboard[] }
  ...
  const getPanelImages = (storyboard: ProjectStoryboard): Array<string | null> => {
  ...
  const getTextPanels = (storyboard: ProjectStoryboard): StoryboardPanel[] => {
    const panels = getStoryboardPanels(storyboard)
    const sortedPanels = [...panels].sort((a: ProjectPanel, b: ProjectPanel) =>
