...
describe('api specific - speaker voice provider persistence', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('returns INVALID_PARAMS when provider is missing in PATCH payload', async () => {
    const mod = await import('@/app/api/projects/[projectId]/speaker-voice/route')
    const req = buildMockRequest({
      path: '/api/projects/project-1/speaker-voice',
      method: 'PATCH',
      body: {
        episodeId: 'episode-1',
        speaker: 'Narrator',
    ...
    expect(prismaMock.projectEpisode.update).not.toHaveBeenCalled()
  })
  ...
  it('stores fal speaker voice with explicit provider and normalized audio storage key', async () => {
    ...
    const updateCall = prismaMock.projectEpisode.update.mock.calls[0] as
